...
### the data file from Niko and Natalie contains 5 columns, 1st is maybe coordination number, 
### 2nd is species = colour coding 3rd to 5th are coordinates; this code just should show the atoms 
### of the different species in e.g. red and blue
...
###### BEGIN of part which reads the datafile and makes a list of lists with 3 floats of it
	...
	try:
		datenNikoNatalie3D = file('out_400K_5at_ed=01_eb=05.txt','r')
		#datenFixedGrid3D = file('datenFixedGrid3D.txt', 'r')
		datenNikoNatalie3DListe = datenNikoNatalie3D.readlines()
		laenge = len(datenNikoNatalie3DListe)
	...
	newCoordsListSpecies1 = []
	newCoordsListSpecies2 = []
	
	for i in range(laenge):
		coord = re_whitespace.split(datenNikoNatalie3DListe[i])
		
		coordCount = float(coord[0])
		species = float(coord[1])
		xCoord = float(coord[2])
		yCoord = float(coord[3])
		zCoord = float(coord[4])	
		
		
		newCoord = [coordCount,species,xCoord,yCoord,zCoord]

		if species == 0:
			newCoordsListSpecies1.append(newCoord)
		elif species == 1:
			newCoordsListSpecies2.append(newCoord)
		else: 
			print 'undefined atom species'
			sys.exit()
...
###### END of part which reads the datafile and makes a list of lists with 3 floats of it and interpolates that
	...
	for i in newCoordsListSpecies1:
		glTranslate(i[2], i[3], i[4])  ### in NikoNatalie output, 3rd to 5th column are coords!
		### glTranslate(x,y,z) translates the matrix which contains all points of the system
		### by vector (x,y,z) 
		
		glutSolidSphere(0.1,16,16)
		glTranslate(-i[2], -i[3], -i[4])
		
	glMaterialfv( GL_FRONT_AND_BACK, GL_AMBIENT, [0.2, 0., 0., 1.0] )
	for i in newCoordsListSpecies2:
		glTranslate(i[2], i[3], i[4])  ### in NikoNatalie output, 3rd to 5th column are coords!
		### glTranslate(x,y,z) translates the matrix which contains all points of the system
		### by vector (x,y,z) 
